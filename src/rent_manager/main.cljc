(ns rent-manager.main
  "Kotodama WASM entrypoint for the Rent_manager clean-room actor (L4) — Clojure port.
  ...
  facts (`rent_manager.<Entity>/<field>`); `*store*` is the in-memory materialization
  used by the contract test and by the WASM runtime before a live engine binds."
  (:require [clojure.string :as str]))

(def ns-prefix "rent_manager")
(def tier "L4")
...
(def entity-specs
  [{:entity "Listing"      :plural "listings"      :id-prefix "rentmana_lis"
   ...
   {:entity "Tenant"       :plural "tenants"        :id-prefix "rentmana_ten"
...
(defn emit-facts
  "EAVT facts for one record: {\"rent_manager.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "rent_manager-compat" :tier tier :entities entities} 200])
...
(defn start! [] :rent_manager-compat/ready)
